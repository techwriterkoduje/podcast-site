...
export async function saveAllEpisodeData() {
  const rssUrl = RSS_URL;

  if (!rssUrl) {
    throw new Error(
      'RSS_URL environment variable is missing. Cannot fetch episode data.'
    );
  }
  ...
  const allEpisodeData = Array.from(items).map((item) => {
  ...
  console.log(
    `📨 Fetched data for ${allEpisodeData.length} episodes from RSS feed.`
  );
  writeFileSync(`all-episodes.json`, JSON.stringify(allEpisodeData, null, 2));
...
export function getOneEpisodeData(
  yearToMatch: string,
  monthToMatch: string,
  dayToMatch: string
): RssItem {
